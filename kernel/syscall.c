...
uint64 sys_user_allocate_page() {
  ...
  uint64 va = g_ufree_page[cur_tp];
  g_ufree_page[cur_tp] += PGSIZE;
  //spinlock_unlock(&print_lock_1);
  user_vm_map((pagetable_t)current[cur_tp]->pagetable, va, PGSIZE, (uint64)pa,
...
uint64 sys_user_free_page(uint64 va) {
  uint64 cur_tp = read_tp();
  user_vm_unmap((pagetable_t)current[cur_tp]->pagetable, va, PGSIZE, 1);
  return 0;
}
