...
ssize_t sys_user_yield() {
  ...
  current->status = READY; //setting the status of currently running process to READY
  insert_to_ready_queue(current); //inserting it in the rear of ready queue
  schedule(); //scheduling a READY process to run.
  return 0;
}
...
ssize_t sys_user_exec(char* pathva){
    ...
    proc = alloc_process(1);
    char * pathname = (char*)user_va_to_pa((pagetable_t)(current->pagetable), pathva);
    int flag = exec(pathname,proc);
    process* tmp = current;
    current = proc;
    free_process(tmp);
    switch_to(current);
